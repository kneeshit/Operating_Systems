...
void dabort_handler (struct trapframe *r)
{
    ...
    // Attempt on-demand page allocation if fault is from a valid user address
    if (proc && fa < proc->sz && fa < UADDR_SZ) {

        // Locate or create the page table entry for the faulting address
        pte_t *pte = walkpgdir(proc->pgdir, (void*)fa, 1);

        // If a PTE exists but the page isn’t mapped yet
        if (pte && !(*pte & PE_TYPES)) {
            ...
            if (page) {
                ...
                memset(page, 0, PTE_SZ);

                // Update the page table entry with physical address and permissions
                *pte = v2p(page) | (AP_KU << 4) | PE_CACHE | PE_BUF | PTE_TYPE;

                // Ensure the MMU uses the new mapping
                flush_tlb();

                // Successfully handled; resume process execution
                return;
            }

            // Could not allocate a page — likely out of memory
            cprintf("on-demand paging failed: out of memory\n");
        }
    }
