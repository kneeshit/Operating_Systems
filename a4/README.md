...
This project implements a series of tests for **on-demand paging** in the ARM version of xv6. The tests cover read/write triggers and fork-based memory isolation.

## Files

- `pt_test2.c` — Contains the main test program.

## Features

- **Read Trigger Test**: Accesses each allocated page to trigger a read.
- **Write Trigger Test**: Writes to each allocated page to trigger a write.
- **Fork Isolation Test**: Ensures that a child process modifying its pages does not affect the parent.
- **On-Demand SBRK Test**: Expands the heap dynamically and runs all tests.
